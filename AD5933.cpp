...
//Mahdi 
bool AD5933::GetGainFactor(double FBResistance, int Freq, int avgNum, double *gainFactor ,double *pShift, bool retStandBy)
{
  ...
  byte numIncrementBackup = avgNum;
  int t1 = 0;
  int rReal, rImag;
  double sumMag = 0, sumPhase = 0;
  for(t1 = 0; t1 < avgNum; t1++){
   if(!getComplexData(&rReal, &rImag))    
    {
      Serial.println("getComplexData error");
      return false;
    }
   sumMag += sqrt( ( pow(rReal,2) + pow(rImag,2)) );
//   Serial.println(rReal);
//   Serial.println(rImag);
//    
//   Serial.print("sumMag: ");
//   Serial.println(sumMag);

   //sumPhase += atan2(rImag, rReal);

    
   if(!setControlMode(CTRL_REPEAT_FREQ)){
      Serial.println("Error in gain factor - cannot set the CTRL_REPEAT_FREQ");
      return false;
      }
   }
  *gainFactor = 1./((sumMag/avgNum)*FBResistance) ;
  
  
 setControlMode(CTRL_POWER_DOWN_MODE);
...
//  and set the cycles 
//  page 25 of the AD5933 datasheet 
bool AD5933::SetSettlingTime(int cycles, int SettlingTime)
{
   int SettlingTimeFlag, SettlingCyclesFlag;
  //set the number of the cycles
  if(sendByte(NUM_SCYCLES_1, 0b00000000))
  {
    SettlingCyclesFlag = 1;
    }
  else // if error 
  {
    SettlingCyclesFlag = 0;
    Serial.println("Error in setting cycle numbers.");
    }
  
  //set the nmberof settling time cycles
    if(sendByte(NUM_SCYCLES_2, SettlingTime)) //if done
  {
    SettlingTimeFlag = 1;
    }
  else //if error 
  {
    SettlingTimeFlag = 0;
    Serial.println("Error in setting settling time cycles.");
    }  
 
  if(SettlingTimeFlag && SettlingCyclesFlag)
    return true;
  else 
    return false; 
}
...
// Mahdi  
// blockread for measurement 
// for increasing the measurement speed 

bool AD5933::Blockread(byte address, int num2Read, byte *toSave)
{
    while((readRegister(STATUS_REG) & STATUS_DATA_VALID) != STATUS_DATA_VALID);
//  if (!sendByte(ADDR_PTR, address))
//    return false;
//  if (!sendByte(BLOCK_READ_CODE, num2Read))
//    return false;
